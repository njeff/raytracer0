...
public class Hittable{
	...
	*/
	...
	public double pdf_value(Vec3 o, Vec3 v){return 0;} //probability distribution of hitting object with this ray
	public Vec3 random(Vec3 o){return new Vec3(1,0,0);} //random in this direction
}
...
class Rotate extends Hittable{
	...
	*/
	...
	public boolean hit(Ray r, double t_min, double t_max, HitRecord rec){
		...
		switch(axis){ //unrotate
			case 0:
				origin.e[1] = cos_theta*r.origin().get(1) - sin_theta*r.origin().get(2);
				...
				break;
			case 1:
				origin.e[0] = cos_theta*r.origin().get(0) - sin_theta*r.origin().get(2);
				...
				break;
			case 2:
				origin.e[0] = cos_theta*r.origin().get(0) - sin_theta*r.origin().get(1);
		...
		if(ptr.hit(rotated_r, t_min, t_max, rec)){
			...
			switch(axis){ //rotate
				case 0:
					p.e[1] = cos_theta*rec.p.get(1) + sin_theta*rec.p.get(2);
					...
					break;
				case 1:
					p.e[0] = cos_theta*rec.p.get(0) + sin_theta*rec.p.get(2);
					...
					break;
				case 2:
					p.e[0] = cos_theta*rec.p.get(0) + sin_theta*rec.p.get(1);
...
class Scale extends Hittable{
	...
	/**
	* @param p object to scale
	* @param f factor to scale by
	*/
	public Scale(Hittable p, double f){
		this(p, f, f, f);
	}

	/**
	* @param p object to scale
	* @param fx factor to scale x by
	* @param fy factor to scale y by
	* @param fz factor to scale z by
	*/
	public Scale(Hittable p, double fx, double fy, double fz){
		ptr = p;
		xf = fx;
		yf = fy;
		zf = fz;
		hasbox = ptr.bounding_box(0,1,bbox);
		bbox.set(new AABB(bbox.min().mul(new Vec3(xf,yf,zf)),bbox.max().mul(new Vec3(xf,yf,zf))));
	}

	public boolean hit(Ray r, double t_min, double t_max, HitRecord rec){
		Vec3 origin = new Vec3();
		origin.set(r.origin());
		origin = origin.mul(new Vec3(1/xf,1/yf,1/zf));

		Ray scaled_r = new Ray(origin, r.direction(), r.time());
		if(ptr.hit(scaled_r, t_min, t_max, rec)){
			rec.p = rec.p.mul(new Vec3(xf,yf,zf));
			return true;
		} else{
			return false;
		}
...
//sorting comparators used for BVHNode axis splitting
